...
class JoystickSensor:
    ...
    gyro = "gyro"
    gravity = "upvector"
    feet_pos = ["FL_foot_pos", "FR_foot_pos"]


@dataclass
class D1HDomainRandConfig(DomainRandConfig):
...
@registry.envcfg("D1HFlat")
@dataclass
class D1HFlatCfg(D1HBaseCfg):
    scene: SceneCfg = field(
        default_factory=lambda: SceneCfg(
            model_file=str(ASSETS_ROOT_PATH / "robots" / "d1h" / "scene_flat.xml")
        )
    )
    ...
    domain_rand: D1HDomainRandConfig = field(default_factory=D1HDomainRandConfig)


class D1HFlatDomainRandomizationProvider(LocomotionDRProvider):
...
@registry.env("D1HFlat", sim_backend="mujoco")
@registry.env("D1HFlat", sim_backend="motrix")
class D1HFlatTask(D1HBaseEnv):
    _cfg: D1HFlatCfg

    def __init__(self, cfg: D1HFlatCfg, num_envs=1, backend_type="mujoco"):
        ...
        self._init_domain_randomization(D1HFlatDomainRandomizationProvider())
        self._backend.set_pre_step_control(self._pre_step_motor_control)
        self._last_dof_vel = np.zeros((num_envs, NUM_D1H_ACTIONS), dtype=get_global_dtype())

    @property
    def obs_groups_spec(self) -> dict[str, int]:
        # gyro(3) + gravity(3) + commands(3) + dof_diff(8) + dof_vel(8) + last_actions(8) = 33
        return {"obs": 33, "critic": 36}
    ...
    def _init_reward_functions(self) -> None:
        self._reward_fns: dict[str, Any] = {
            ...
            "dof_thigh_vel": self._reward_dof_thigh_vel,
            "collision": self._reward_collision,
            "body_pos_to_feet_x": self._reward_body_pos_to_feet_x,
            "body_feet_distance_x": self._reward_body_feet_distance_x,
            "body_feet_distance_y": self._reward_body_feet_distance_y,
            "body_symmetry_y": self._reward_body_symmetry_y,
            "body_symmetry_z": self._reward_body_symmetry_z,
            "collision_head": self._reward_collision_head,
        }
    ...
    def _reward_dof_acc(self, ctx: RewardContext) -> np.ndarray:
        if not hasattr(self, "_last_dof_vel"):
            self._last_dof_vel = np.zeros((self._num_envs, NUM_D1H_ACTIONS), dtype=get_global_dtype())
    ...
    def _reward_dof_thigh_vel(self, ctx: RewardContext) -> np.ndarray:
        dof_vel = ctx.info.get("dof_vel", np.zeros((self._num_envs, NUM_D1H_ACTIONS), dtype=get_global_dtype()))
        thigh_indices = [1, 5]
        return np.sum(np.square(dof_vel[:, thigh_indices]), axis=1)

    def _reward_collision(self, ctx: RewardContext) -> np.ndarray:
        return np.zeros(self._num_envs, dtype=get_global_dtype())

    def _reward_collision_head(self, ctx: RewardContext) -> np.ndarray:
        return np.zeros(self._num_envs, dtype=get_global_dtype())

    def _reward_body_pos_to_feet_x(self, ctx: RewardContext) -> np.ndarray:
        return np.zeros(self._num_envs, dtype=get_global_dtype())

    def _reward_body_feet_distance_x(self, ctx: RewardContext) -> np.ndarray:
        return np.zeros(self._num_envs, dtype=get_global_dtype())

    def _reward_body_feet_distance_y(self, ctx: RewardContext) -> np.ndarray:
        return np.zeros(self._num_envs, dtype=get_global_dtype())

    def _reward_body_symmetry_y(self, ctx: RewardContext) -> np.ndarray:
        return np.zeros(self._num_envs, dtype=get_global_dtype())

    def _reward_body_symmetry_z(self, ctx: RewardContext) -> np.ndarray:
        return np.zeros(self._num_envs, dtype=get_global_dtype())
