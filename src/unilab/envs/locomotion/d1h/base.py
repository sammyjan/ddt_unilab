...
NUM_D1H_ACTIONS = len(D1H_JOINT_NAMES)

D1H_FOOT_INDICES = np.asarray([3, 7], dtype=np.int32)
D1H_HIP_INDICES = np.asarray([0, 4], dtype=np.int32)
D1H_LEG_INDICES = np.asarray(
    [i for i in range(NUM_D1H_ACTIONS) if i not in D1H_FOOT_INDICES],
    dtype=np.int32,
)

DEFAULT_D1H_ANGLES = np.asarray(
    [
        0.0, 0.8, -1.5, 0.0,   # FL
        0.0, 0.8, -1.5, 0.0,   # FR
    ],
...
class D1HBaseEnv(LocomotionBaseEnv):
    ...
    def _init_buffers(self) -> None:
        super()._init_buffers()
        self.default_angles = np.asarray(DEFAULT_D1H_ANGLES, dtype=self.default_angles.dtype)
    ...
    def apply_action(self, actions: np.ndarray, state: NpEnvState) -> np.ndarray:
        ...
        actions_scaled[:, D1H_HIP_INDICES] *= self._cfg.control_config.hip_scale_reduction
    ...
    def _pre_step_motor_control(self, backend, policy_ctrl: np.ndarray) -> np.ndarray:
        ...
        torque[:, D1H_LEG_INDICES] = (
            kp * (policy_ctrl[:, D1H_LEG_INDICES] - joint_pos[:, D1H_LEG_INDICES])
            - kd * joint_vel[:, D1H_LEG_INDICES]
        )
        torque[:, D1H_FOOT_INDICES] = (
            kp_foot * (policy_ctrl[:, D1H_FOOT_INDICES] - joint_pos[:, D1H_FOOT_INDICES])
        )
        if kd_foot > 0:
            torque[:, D1H_FOOT_INDICES] -= kd_foot * joint_vel[:, D1H_FOOT_INDICES]
